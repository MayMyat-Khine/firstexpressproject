...
import { createOrderService, updateOrder, getOrders, getOrderById, getOrderByBranch, getOrdersByCustomer, getMyOrderById } from "../services/order.service.js";
...
export async function orderGetAllController(req, res, next) {
    try {
        ...
        const { orders, pagination } = await getOrders(validateData);
        res.json({ success: true, body: orders, pagination });
...
export async function orderGetMyOrdersController(req, res, next) {
    try {
        ...
        var validateData = matchedData(req);
        const { orders, pagination } = await getOrdersByCustomer(customerId, validateData);
        res.json({ success: true, body: orders, pagination });
